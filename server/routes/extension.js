import { localOrigin } from "../utils/security.js";
import { lastExtension } from "../utils/extensionState.js";
/**
...
function currentExtensionDir() {
  try {
    ...
    return { dir: path.join(repoRoot, "extension"), staged: false };
  }
}
...
function nativeHostRegistered() {
    ...
    return [
      ...
      (browser) =>
        !spawnSync(
          ...
          [
            "query",
            `HKCU\\Software\\${browser}\\NativeMessagingHosts\\com.mediagrab.host`,
            "/ve",
          ],
  ...
  return dirs.some((d) => {
    try {
      return fs.existsSync(path.join(d, "com.mediagrab.host.json"));
    } catch {
      return false;
    }
  });
...
router.get("/info", (req, res) => {
  ...
  res.json({
    lastHandshake: lastExtension,
    bundledVersion: JSON.parse(
      fs.readFileSync(
    ...
    reloadRequired: "Chrome 更新外掛後須按重新載入；外掛視窗顯示實際載入版號",
    ...
    tokenPresent: fs.existsSync(
      path.join(os.homedir(), ".mediagrab", "capture-token"),
    ),
...
router.post("/reveal", localOnly, (req, res) => {
  ...
  if (process.platform === "darwin") {
    cmd = "open";
    args = ["-R", extensionDir];
  } // reveal, folder selected
  else if (process.platform === "win32") {
    cmd = "explorer";
    args = ["/select,", extensionDir];
  } else {
    cmd = "xdg-open";
    args = [extensionDir];
  }
