import { noteExtension } from "../utils/extensionState.js";
...
const SEGMENT_HINT =
  /(?:^|\/)(?:seg|frag|chunk|init)[-_.]?\d*|[_-]seg\d|\/range\/|\.m4s(\?|$)|_\d{2,}\.mp4(\?|$)/i;
...
router.post("/download", (req, res, next) => {
  try {
    ...
    const hasManifest =
      typeof manifestUrl === "string" && /^https?:\/\//i.test(manifestUrl);
    ...
    if (!hasManifest && segs.length === 0) {
      return res
        .status(400)
        .json({ error: "manifestUrl or segmentUrls is required" });
    }
    ...
    if (hasManifest) {
      ...
      const mp4 = segs.find(
        (u) => /\.mp4(\?|$)/i.test(u) && !SEGMENT_HINT.test(u),
      );
      if (mp4) {
        dlUrl = mp4;
        via = "progressive-mp4";
      } else {
        // Segment order/duration/initialization are unknown: never label as full VOD.
      }
    }
    ...
    if (!dlUrl) {
      ...
      return res.status(501).json({
        error:
          "只擷取到片段、無法安全重組（缺少播放清單或初始化段）。請改用有 manifest 的播放器，或用擴充的「錄製模式」下載。",
    ...
    if (via === "manifest") {
      ...
      if (wid) {
        if (!/\.m3u8(\?|$)/i.test(manifestUrl))
          dlUrl = `https://fast.wistia.com/embed/medias/${wid}`;
    ...
    const task = taskManager.createTask({
      ...
      providerPage: !!wistiaId(dlUrl) && !/\.m3u8(?:\?|$)/i.test(dlUrl),
      useCapturedHeaders: true,
      headers: reqHeaders,
      contexts: req.body.contexts,
      capturedCookies: req.body.cookies,
      referer: dlReferer,
